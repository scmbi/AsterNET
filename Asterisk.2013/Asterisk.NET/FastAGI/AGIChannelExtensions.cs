...
namespace AsterNET.FastAGI
{
    ...
    public static class AGIChannelExtensions
    {
		...
		public static string GetData(this AGIChannel source, string file)
		{
			AGIReply lastReply = source.SendCommand(new GetDataCommand(file));
		...
		/// <returns> a String containing the DTMF the user entered</returns>
		public static string GetData(this AGIChannel source, string file, long timeout)
		{
			AGIReply lastReply = source.SendCommand(new GetDataCommand(file, timeout));
			return lastReply.GetResult();
		}
		...
		/// <returns> a String containing the DTMF the user entered</returns>
		public static string GetData(this AGIChannel source, string file, long timeout, int maxDigits)
		{
			AGIReply lastReply = source.SendCommand(new GetDataCommand(file, timeout, maxDigits));
			return lastReply.GetResult();
		}
		...
		{
			AGIReply lastReply = source.SendCommand(new GetOptionCommand(file, escapeDigits, timeout));
			return lastReply.ResultCodeAsChar;
		}
		...
		{
			AGIReply lastReply = source.SendCommand(new ExecCommand(application));
			return lastReply.ResultCode;
		}
		...
		{
			AGIReply lastReply = source.SendCommand(new ExecCommand(application, options));
			return lastReply.ResultCode;
		}
		...
		public static void SetVariable(this AGIChannel source, string name, string val)
		{
		...
		/// <returns>the value of the given variable or null if not set.</returns>
		public static string GetFullVariable(this AGIChannel source, string name, string channelName)
		{
			AGIReply lastReply = source.SendCommand(new GetFullVariableCommand(name, channelName));
			if (lastReply.ResultCode != 1)
				return null;
			return lastReply.Extra;
		}
		...
		{
			AGIReply lastReply = source.SendCommand(new ControlStreamFileCommand(file, escapeDigits, offset, forwardDigit, rewindDigit, pauseDigit));
			return lastReply.ResultCode;
		}
		#endregion
	}
